...
@interface BZRecordHeartRateController ()<UIPickerViewDelegate ,UIPickerViewDataSource>
...
@implementation BZRecordHeartRateController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)setCustomUI{
    ...
    // pickview
    _array = @[@"静息心率",@"运动前",@"运动后"];
    _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(deleteBtn.frame) + 10, kMainWidth, kMainWidth * 0.5)];
...
- (void)deleteClick{
    NSMutableDictionary *args = [NSMutableDictionary dictionary];
    args[@"id"] = _hrId;
    [httpUtil doPostRequest:@"api/medicalApiController/delHeartRateRecord" args:args targetVC:self response:^(ResponseModel *responseMd) {
        if (responseMd.isResultOk) {
            // 跳转控制器
            [self.navigationController popViewControllerAnimated:YES];
        }
